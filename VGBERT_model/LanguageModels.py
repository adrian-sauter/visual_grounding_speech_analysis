...
class Bert_object_hidden(nn.Module):
    def __init__(self, embedding_dim=768, dropout_rate=0.1, output_hidden=False):
        super(Bert_object_hidden, self).__init__()
        config = BertConfig.from_pretrained('bert-base-uncased', output_hidden_states=output_hidden)
        config.hidden_dropout_prob = dropout_rate
        config.attention_probs_dropout_prob = dropout_rate
        seed_model = BertModel(config)
        base_model = BertModel.from_pretrained('bert-base-uncased', output_hidden_states=output_hidden)
        base_model.to('cpu')
        # load weight from base model to seed model
        new_state_dict = OrderedDict()
        for k, v in base_model.state_dict().items():
