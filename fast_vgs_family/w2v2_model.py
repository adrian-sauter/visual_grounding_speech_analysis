...
class Wav2Vec2Model_cls(BaseFairseqModel):
    ...
    def forward(self, source, padding_mask=None, mask=True, features_only=False, superb=False, tgt_layer=None):
        ...
        #print(f'tgt_layer right before encoder is called in line 737 is {tgt_layer}')
...
class TransformerEncoder(nn.Module):
    ...
    def forward(self, x, padding_mask=None, superb=False, tgt_layer=None):
        if superb:
            ...
            return all_feats
        #print(f'tgt_layer in line 982 right before extract_features is {tgt_layer}')
        x, layer_feats = self.extract_features(x, padding_mask, tgt_layer=tgt_layer)
    ...
    def extract_features(self, x, padding_mask=None, need_head_weights=False, tgt_layer=None, all_hidden_states=False):
        if tgt_layer == None:
            #print(f'tgt_layer is somehow None in extract_features')
            layer_use = self.layer_use
            stop_pass = False
        else:
            #print(f'tgt_layer in extract_features is: {tgt_layer}')
            layer_use = tgt_layer
            stop_pass = True
